...
def _run_record(seed: int, variant: str, run_dir: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "lift_accept_rate": float(summary.get("region_lift_accept_rate", 0.0)),
...
def _build_config(
    *,
    ...
    if variant in {"strict_baseline", "parego_baseline"}:
        ...
        elif variant in {"warmstart_region_lifted_gp_guarded_pool", "warmstart_region_lifted_gp_force_pool_tuned"}:
            cfg.update(
                {
                    "warmstart_cache_mode": "read",
                    "warmstart_cache_use_selected": True,
...
def _gate_conditions_met(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    region = aggregates.get("warmstart_region_lifted_gp") or {}
    delta_vs_strict = float(
        (comparisons.get("warmstart_region_lifted_gp_vs_strict_baseline") or {}).get("mean_canonical_hv_delta", -1.0)
    )
    delta_vs_plain = float(
        (comparisons.get("warmstart_region_lifted_gp_vs_warmstart_plain_ei") or {}).get("mean_canonical_hv_delta", -1.0)
    )
    region_diag_total = int(region.get("plain_candidate_inside_region_count_total", 0)) + int(
        region.get("diagnostic_override_candidate_count_total", 0)
    )
    passed = bool(delta_vs_strict > 0.0 and delta_vs_plain >= 0.0 and region_diag_total > 0)
    return {
        "passed": passed,
        "delta_vs_strict": delta_vs_strict,
        "delta_vs_plain": delta_vs_plain,
        "region_diag_total": region_diag_total,
    }
...
def main() -> None:
    ...
    llm_variants = [variant for variant in args.variants if variant != "strict_baseline"]
