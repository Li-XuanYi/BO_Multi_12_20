"""
plot_nsga2_ecker_oregan.py — NSGA-II在Ecker2015和ORegan2022上的HV表现对比
=============================================================================
绘制两张图：
1. NSGA-II + Ecker2015 的HV收敛曲线（绿色实线 + 阴影带）
2. NSGA-II + ORegan2022 的HV收敛曲线（绿色实线 + 阴影带）

Usage:
    python Compare_Exp/plot_nsga2_ecker_oregan.py
"""
...
def _load_multiseed_data(
    ...
    目录结构：
    root/seed0/nsga2_param_set/summary.json
    root/seed1/nsga2_param_set/summary.json
    ...
    """
    traces = []
    for s in range(n_seeds):
        seed_dir = root / f"seed{s}" / f"nsga2_{param_set}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot NSGA-II performance on Ecker2015 and ORegan2022"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    experiments = [
        {
            "name": "Ecker2015",
            "param_set": "Ecker2015",
            "root": Path("optimized_experiments/nsga2_Ecker2015_5seeds_56evals_2026_05_09"),
            "output": args.output_dir / "nsga2_ecker2015_hv_50iter",
        },
        {
            "name": "ORegan2022",
            "param_set": "ORegan2022",
            "root": Path("optimized_experiments/nsga2_ORegan2022_5seeds_56evals_2026_05_09"),
            "output": args.output_dir / "nsga2_oregan2022_hv_50iter",
        },
    ]
