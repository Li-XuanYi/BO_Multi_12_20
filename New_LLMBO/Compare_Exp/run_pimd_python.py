"""
run_pimd_python.py - Run PIMD experiments with Python native implementation
============================================================================

Usage:
    python Compare_Exp/run_pimd_python.py

This runs PIMD algorithm with seeds 8409-8413, 50 evaluations each.
...
from Compare_Exp.Exp.disk_pimd_algorithms import PIMDOptimizer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PIMD experiments (Python native)")
    ...
    parser.add_argument("--wmax", type=int, default=15)
...
def run_single(seed: int, args: argparse.Namespace, output_root: Path) -> Dict[str, Any]:
    ...
    output_dir = output_root / f"seed{seed}" / "pimd_Chen2020"
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info(f"Starting PIMD with seed {seed}")

    optimizer = PIMDOptimizer(
        seed=seed,
...
def main() -> int:
    ...
    if args.output_root is None:
        ...
        args.output_root = (
            ...
            / "Compare_Exp"
            / "experiment_records"
            / f"pimd_python_{args.param_set}_5seeds_{args.n_evals}evals_{date_str}"
        )

    args.output_root.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 60)
    logger.info("PIMD Experiments (Python Native)")
    logger.info("=" * 60)
