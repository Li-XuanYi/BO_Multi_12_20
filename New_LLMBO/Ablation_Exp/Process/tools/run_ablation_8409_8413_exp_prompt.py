"""
消融实验（使用新版 Experimental WarmStart Prompt）
- seeds: 8409-8413 (5 seeds)
- 50 iterations
- Model: deepseek-v3
- 4-way comparison: Baseline, WarmStart(exp), LLM_Region, LLMBO-MO
"""
...
DEFAULT_MODEL = "deepseek-v3"  # Non-thinking version
...
VARIANTS: List[VariantSpec] = [
    ...
    VariantSpec(
        ...
        overrides={
            "n_warmstart": 6,
            "n_random_init": 0,
            "warmstart_prompt_version": "experimental",  # Use new experimental prompt
        },
        warmstart_cache_mode="disabled",  # Disable cache to ensure fair comparison
...
LGBO_SMOKE_VARIANTS: List[VariantSpec] = [
    VariantSpec(
        key="lgbo_region",
        label="LGBO Proposition1",
        preset="warmstart_region_lgbo_proposition1",
        needs_llm=True,
        color="#6C5CE7",
        overrides={
            "n_warmstart": 0,
    ...
    VariantSpec(
        key="warmstart_lgbo",
        label="WarmStart+LGBO Proposition1",
        preset="warmstart_region_lgbo_proposition1",
        needs_llm=True,
        color="#00A676",
        overrides={
...
LLM_REGION_VS_BASELINE_VARIANTS: List[VariantSpec] = [
    VARIANTS[0],
    LGBO_SMOKE_VARIANTS[0],
]

LLM_REGION_VS_BASELINE_20ITER_VARIANTS: List[VariantSpec] = [
    VARIANTS[0],
    VariantSpec(
        key="llm_region",
        label="LLM_Region",
...
LLM_REGION_DAMPED_VS_BASELINE_VARIANTS: List[VariantSpec] = [
    VARIANTS[0],
    VariantSpec(
        key="lgbo_region_damped",
        label="LLM_Region LGBO Damped",
        preset="warmstart_region_lgbo_proposition1",
        needs_llm=True,
        color="#8E7CC3",
        overrides={
            ...
            "region_lift_external_influence_mode": "diagnostic_only",
            "region_lift_apply_override": False,
            "region_lift_lgbo_min_variance": 400.0,
        },
    ),
]

BASELINE_LLM_REGION_LLMBO_BO_VARIANTS: List[VariantSpec] = [
    VARIANTS[0],
    VariantSpec(
        key="llm_region",
        label="LLM_Region",
...
LLM_REGION_ADAPTIVE_SMOKE_VARIANTS: List[VariantSpec] = [
    VARIANTS[0],
    VariantSpec(
        key="llm_region_old",
        label="LLM_Region old posterior",
        preset="llm_region_lgbo_posterior",
        needs_llm=True,
        color="#6C5CE7",
        overrides={"n_warmstart": 0, "n_random_init": 6, "region_lift_active_until": 10},
    ),
    VariantSpec(
        key="llm_region_calibrated",
        label="LLM_Region calibrated",
        preset="llm_region_lgbo_posterior_calibrated",
        needs_llm=True,
        color="#8E7CC3",
    ),
    VariantSpec(
        key="llm_region_adaptive",
        label="LLM_Region adaptive",
        preset="llm_region_lgbo_posterior_adaptive",
        needs_llm=True,
        color="#00A676",
    ),
]

ADAPTIVE_4WAY_VARIANTS: List[VariantSpec] = [
    VARIANTS[0],
    VARIANTS[1],
    VariantSpec(
        key="baseline_llm_region",
        label="Baseline+LLM_Region adaptive",
        preset="llm_region_lgbo_posterior_adaptive",
        needs_llm=True,
        color="#F0A33A",
        overrides={"n_warmstart": 0, "n_random_init": 6},
    ...
    VariantSpec(
        key="llmbo_mo",
        label="WarmStart+LLM_Region adaptive",
        preset="llm_region_lgbo_posterior_adaptive",
        needs_llm=True,
        color="#D45162",
        overrides={
...
WARMSTART_LLMBO_PAIRED_VARIANTS: List[VariantSpec] = [
    VariantSpec(
        key="baseline_warmstart",
        label="WarmStart paired",
        preset="warmstart_plain_ei",
        needs_llm=True,
        color="#2E8BC8",
        overrides={
    ...
    VariantSpec(
        key="llmbo_mo",
        label="WarmStart+LLM_Region adaptive paired",
        preset="llm_region_lgbo_posterior_adaptive",
        needs_llm=True,
        color="#D45162",
        overrides={
...
def _selected_variants(args: argparse.Namespace) -> List[VariantSpec]:
    variant_set = str(getattr(args, "variant_set", "heuristic4") or "heuristic4").lower()
    if variant_set == "lgbo_smoke":
        return LGBO_SMOKE_VARIANTS
    if variant_set == "llm_region_vs_baseline":
        return LLM_REGION_VS_BASELINE_VARIANTS
    if variant_set == "llm_region_vs_baseline_20iter":
        return LLM_REGION_VS_BASELINE_20ITER_VARIANTS
    if variant_set == "llm_region_damped_vs_baseline":
        return LLM_REGION_DAMPED_VS_BASELINE_VARIANTS
    if variant_set == "baseline_llm_region_llmbo_bo":
        return BASELINE_LLM_REGION_LLMBO_BO_VARIANTS
    if variant_set == "llm_region_adaptive_smoke":
        return LLM_REGION_ADAPTIVE_SMOKE_VARIANTS
    if variant_set == "adaptive4":
        return ADAPTIVE_4WAY_VARIANTS
    if variant_set == "warmstart_llmbo_paired":
        return WARMSTART_LLMBO_PAIRED_VARIANTS
    return VARIANTS
...
def _default_output_root() -> Path:
    ...
    return (
        ...
        / "Ablation_Exp"
        / "experiment_records"
        / f"ablation_4way_5seeds_{DEFAULT_ITERATIONS}iter_{DEFAULT_MODEL}_exp_prompt_{date_tag}"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run four-way ablation experiments with experimental warmstart prompt (seeds 8409-8413)."
    )
    ...
    parser.add_argument(
        ...
        choices=[
            "heuristic4",
            "lgbo_smoke",
            "llm_region_vs_baseline",
            "llm_region_vs_baseline_20iter",
            "llm_region_damped_vs_baseline",
            "baseline_llm_region_llmbo_bo",
            "llm_region_adaptive_smoke",
            "adaptive4",
            "warmstart_llmbo_paired",
        ],
        default="heuristic4",
        help=(
            "heuristic4 preserves the historical 4-way ablation; "
            "lgbo_smoke runs the new Proposition-1 LGBO branch; "
            "llm_region_vs_baseline compares Baseline against region-only LGBO; "
            "llm_region_vs_baseline_20iter keeps region lift active for 20-iteration runs; "
            "llm_region_damped_vs_baseline tests a conservative LGBO scale; "
            "baseline_llm_region_llmbo_bo runs the requested three-way comparison; "
            "llm_region_adaptive_smoke compares old/calibrated/adaptive standalone Region; "
            "adaptive4 compares Baseline, WarmStart, adaptive Region, and WarmStart+adaptive Region; "
            "warmstart_llmbo_paired compares WarmStart and LLMBO with shared selected warmstart points."
        ),
    )
    parser.add_argument(
        "--llm-backend",
        ...
        help="Use mock for algorithmic smoke tests that should not consume API quota.",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
...
def _build_config(
    *,
    ...
    cfg: Dict[str, Any] = {
        ...
        "random_init_cache_path": None,  # Disable cache to ensure fair comparison
        ...
        cfg.update(
            {
                ...
                "llm_api_key": api_key if str(llm_backend) != "mock" else "",
                "llm_n_samples": 1,
                "llm_temperature": 0.0,
            ...
            cfg.update(
                {
                    ...
                    "warmstart_cache_mode": spec.warmstart_cache_mode or "disabled",
...
def _record_for_run(seed: int, spec: VariantSpec, run_dir: Path) -> Dict[str, Any]:
    ...
    if summary_path.exists():
        ...
        lift_accept_rate = float(summary.get("region_lift_accept_rate", summary.get("lift_accept_rate", 0.0)) or 0.0)
        return {
            **base,
            "status": "ok",
            ...
            "region_lift_accept_rate": lift_accept_rate,
...
def _write_manifest(output_root: Path, args: argparse.Namespace, variants: Sequence[VariantSpec]) -> None:
    payload = {
        "created_at": datetime.now().isoformat(),
        "experiment": "ablation_4way_exp_prompt",
        "variant_set": str(getattr(args, "variant_set", "heuristic4")),
        "iterations": int(args.iterations),
        "seeds": [int(seed) for seed in args.seeds],
        "api_base": str(args.api_base),
        "model": str(args.model),
        "model_display": canonical_model_label(str(args.model)),
        "llm_backend": str(getattr(args, "llm_backend", "openai")),
...
def _write_report(output_root: Path, args: argparse.Namespace, variants: Sequence[VariantSpec]) -> Dict[str, Any]:
    seeds = [int(seed) for seed in args.seeds]
    ...
    if "warmstart_lgbo" in by_variant and "lgbo_region" in by_variant:
        comparisons["warmstart_lgbo_vs_lgbo_region"] = _comparison(
            by_variant["warmstart_lgbo"], by_variant["lgbo_region"]
        )
...
def _plot_hv_box(report: Mapping[str, Any], image_dir: Path, variants: Sequence[VariantSpec]) -> Dict[str, str]:
    import matplotlib
...
def generate_plots(
    ...
    artifacts = {
        "hv_box": _plot_hv_box(report, image_dir, variants),
        "hv_convergence": _plot_hv_convergence(output_root, args.seeds, image_dir, variants),
        "pareto_3d": _plot_pareto_3d(output_root, args.seeds, image_dir, variants),
    }
    _write_plot_config(report, output_root, image_dir, artifacts, variants)
    return artifacts
...
def main() -> int:
    ...
    needs_real_api = any(spec.needs_llm for spec in variants) and str(args.llm_backend) != "mock"
    if not args.summarize_only and needs_real_api and not api_key:
        raise RuntimeError("Set LLM_API_KEY or OPENAI_API_KEY before running real LLM ablation experiments.")

    _write_manifest(output_root, args, variants)
    seeds = [int(seed) for seed in args.seeds]
    ...
    report = _write_report(output_root, args, variants)
    artifacts = generate_plots(output_root, args, report, variants)
    print(
        json.dumps(
            {
                "event": "done",
