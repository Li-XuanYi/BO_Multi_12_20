...
def _extract_iteration_trace(summary: Dict[str, Any], max_iteration: int) -> Optional[Dict[str, np.ndarray]]:
    ...
    for item in hv_trace:
        ...
        if iteration < 0 or iteration > int(max_iteration):
            continue
        by_iter[iteration] = float(item.get("canonical_hv", item.get("hypervolume_canonical", 0.0)))

    if not by_iter:
        return None

    xs = np.array(sorted(by_iter), dtype=int)
    ys = np.array([by_iter[int(x)] for x in xs], dtype=float)
    return {"x": xs, "hv": ys}
...
def _stack_mode(summary_paths: List[Path], max_iteration: int) -> Optional[Dict[str, Any]]:
    ...
    for path in summary_paths:
        if not path.exists():
            continue
        trace = _extract_iteration_trace(_load_json(path), max_iteration=max_iteration)
...
def plot(report_path: Path, exp_root: Path, output_dir: Path, modes: List[str], max_iteration: int) -> Dict[str, Any]:
    ...
    if y_values:
        ...
        y_max = min(1.0, float(np.ceil((y_all.max() + 0.01) * 100.0) / 100.0))
        if y_max > y_min:
    ...
    outputs = {}
    for ext in ("png", "pdf"):
        path = output_dir / f"scalarization_hv_comparison.{ext}"
