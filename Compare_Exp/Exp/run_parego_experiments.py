"""
run_parego_experiments.py — Multi-seed ParEGO experiment launcher
=================================================================
Runs ParEGO with multiple random seeds, saves per-seed results and
an aggregated report.json for paper comparison.

Supports two variants:
- baseline: Original ParEGO with Riesz s-energy weights
- matlab_reference: MATLAB-style ParEGO with Das-Dennis weights
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run multi-seed ParEGO experiments")
    parser.add_argument(
    ...
    parser.add_argument(
        "--n-random-init", type=int, default=6,
        help="Number of random initialization points",
    )
    parser.add_argument(
        "--variant", type=str, default="matlab_reference",
        choices=["baseline", "matlab_reference"],
        help="ParEGO variant to use",
    )
    parser.add_argument(
...
def run_single(seed: int, n_evals: int, n_random_init: int, variant: str, param_set: str, output_dir: Path) -> Dict:
...
def _seed_dir(root: Path, seed: int, variant: str, param_set: str) -> Path:
    return root / f"seed{seed}" / f"parego_{variant}_{param_set}"


def build_report(records: List[Dict], seeds: List[int], n_evals: int, variant: str, param_set: str) -> Dict:
        ...
        "records": [
            {
                ...
                "summary_path": str(_seed_dir(Path("."), r["seed"], variant, param_set)),
            }
            for r in records
        ],
...
def main():
    ...
    if args.output_root is None:
        ...
        args.output_root = PROJECT_ROOT / "optimized_experiments" / f"parego_{args.variant}_{args.param_set}_{len(args.seeds)}seeds_{args.n_evals}evals_{date_str}"
    ...
    dir_fn = lambda s: _seed_dir(output_root, s, args.variant, args.param_set)
    records: List[Dict] = []

    for seed in args.seeds:
        seed_dir = dir_fn(seed)
        try:
            summary = run_single(seed, args.n_evals, args.n_random_init, args.variant, args.param_set, seed_dir)
    ...
    report = build_report(records, args.seeds, args.n_evals, args.variant, args.param_set)

    # Fix summary_path to be relative
    for rec in report["records"]:
        rec["summary_path"] = str(output_root / f"seed{rec['seed']}" / f"parego_{args.variant}_{args.param_set}" / "summary.json")
    ...
    print(f"\n{'='*50}")
    print(f"ParEGO-{args.variant}-{args.param_set} Results ({len(records)} seeds)")
    print(f"{'='*50}")
