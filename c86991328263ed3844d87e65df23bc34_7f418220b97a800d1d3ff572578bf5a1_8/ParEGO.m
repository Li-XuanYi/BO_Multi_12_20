function DB = ParEGO(init_pop)
    tic;
    % Parameter Setting
    dim = 9;  % 仅修改：5→9（适配9维：4SOC+5电流）
    max_iter = 180;
    obj_num = 3;
    up = ones(1,9);  % 仅修改：5→9
    dn = zeros(1,9); % 仅修改：5→9
    [n_w,W] = EqualWeight(30,3);
    W = W + 0.000001;
    % Initialize Dataset
%     DB.x = (up - dn).*rand(20,dim) + dn;
    DB.x = init_pop;
    DB.y = Evaluate_Bat(DB.x,0.7);
    DB.time = zeros(max_iter,1);
    for iter = 1:max_iter
        % Print HV values
...
%         fprintf([num2str(HV(iter)),' \n'])
        ...
        y_agg_norm = (y_agg - mean(y_agg))./std(y_agg);                     % 值归一化到-1和1之间
