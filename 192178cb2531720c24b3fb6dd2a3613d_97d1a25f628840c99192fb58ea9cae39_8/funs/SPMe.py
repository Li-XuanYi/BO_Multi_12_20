...
# soc 使用
def cal_soc(model_type, c):
    if model_type=="Mohtat2020":
        return (c - 43.4595149467885) / (23956.17694372911 - 43.4595149467885)
    elif model_type=="Ecker2015":
        return (c - 113.42352193587662) / (27081.507595095063 - 113.42352193587662)
    elif model_type=="ORegan2022":
        return (c - 874.6094493877572) / (29582.81857062898 - 872.9651389896292)
    else:
        return (c - 872.9651389896292) / (30171.311359086325 - 872.9651389896292)


class SPM:
    def __init__(self, init_v=2.8, init_t=298.15, SOH=1.0, model_type="ours"):
        # 传递参数
        self.reward = None
        ...
        if model_type=="ours":
            ...
            param = pybamm.ParameterValues(model_type)
            param.update(
                {"Current function [A]": "[input]",
                 "Upper voltage cut-off [V]": 4.4})
            param.set_initial_stoichiometries("{} V".format(init_v))
        ...
        self.temp = init_t
        self.voltage = init_v
        self.soc = cal_soc(model_type, param["Initial concentration in negative electrode [mol.m-3]"])
        self.soc_d = None
        self.temp_d = None
        self.voltage_d = None
        self.sol = None
    ...
    def step(self, action, st=None):
        ...
        self.soc = cal_soc(self.model_type, c)
        self.info = sol.termination
        # 数据的更新
        self.voltage_d = sol["Voltage [V]"].entries
        self.temp_d = sol["X-averaged cell temperature [K]"].entries
        self.soc_d = cal_soc(self.model_type, sol["R-averaged negative particle concentration [mol.m-3]"].entries[-1])
        self.sol = sol
        self.step_cnt += 1
        # 状态打印
