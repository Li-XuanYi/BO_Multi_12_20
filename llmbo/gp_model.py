...
class LLMPreferenceCoupling:
    ...
    # Legacy serialized/manual couplings omitted this field and used the prior
    # kernel. New RegionLift builders pass an explicit, safe source.
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "max_shift_std": None if self.max_shift_std is None else float(self.max_shift_std),
...
class MaternGPModel:
    ...
    def predict_with_coupling(
        ...
        max_shift_std = getattr(coupling, "max_shift_std", None)
        if max_shift_std is not None and np.isfinite(float(max_shift_std)) and float(max_shift_std) > 0.0:
            shift_z = np.clip(shift_z, -float(max_shift_std), float(max_shift_std))
        _, y_std = self.target_standardization()
