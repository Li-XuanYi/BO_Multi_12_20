...
class LLMCaller:
    ...
    def _coerce_text_part(value: Any) -> str:
        """Convert provider-specific content blocks to plain response text."""
        if value is None:
            return ""
        ...
        if isinstance(value, Mapping):
            ...
            for key in ("text", "content", "value"):
                if key in value:
                    part = LLMCaller._coerce_text_part(value.get(key))
                    if part:
                        pieces.append(part)
            return "\n".join(pieces)
        if isinstance(value, (list, tuple)):
            return "\n".join(
                part
                for part in (LLMCaller._coerce_text_part(item) for item in value)
                if part
            )
    ...
    def _extract_message_text(cls, message: Any) -> Tuple[str, str]:
        # reasoning_content is hidden reasoning, not the structured answer.
    ...
    def _build_openai_diagnostic(
        ...
        reasoning_text = cls._coerce_text_part(
            cls._message_get(message, "reasoning_content")
        )
        ...
        *,
        ...
        for i in range(n):
            ...
            for attempt in range(self._cfg.request_retries + 1):
                try:
                    logger.debug(
                        "LLM request %d/%d attempt %d | model=%s max_tokens=%d prompt_len=%d",
                        i + 1, n, attempt + 1,
                        self._cfg.model,
                        4096 if max_tokens is None else int(max_tokens),
                        len(prompt),
                    )
                    logger.debug("LLM prompt (first 1000 chars): %.1000s", prompt)
                    ...
                    if self._cfg.thinking_mode is not None:
                        request_kwargs["extra_body"] = {
                            "thinking": {"type": self._cfg.thinking_mode}
                        }
                    ...
                    self.last_call_diagnostics.append(
                        self._build_openai_diagnostic(
                            resp, choice, message, content, source
                        )
                    )
                    usage = getattr(resp, "usage", None)
                    logger.debug(
                        "LLM response %d/%d | content_len=%d finish_reason=%s usage=%s",
                        i + 1, n, len(content),
                    ...
                    logger.debug("LLM raw response (full): %s", content)
        ...
        for i in range(n):
            ...
            for attempt in range(self._cfg.request_retries + 1):
                try:
                    ...
                    content = self._coerce_text_part(getattr(resp, "content", None)).strip()
                    ...
                    self.last_call_diagnostics.append(
                        {
                            "backend": "anthropic",
                            "model": getattr(resp, "model", self._cfg.model),
...
class LLMInterface:
    ...
    def _extract_json_flexible(self, text: str) -> Optional[Any]:
        """Extract one JSON payload while tolerating common model wrappers."""
        if not text or not text.strip():
            return None
        stripped = ResponseParser._strip_thinking_tags(text)
        if not stripped:
            return None

        parsed = self._parser.extract_json(stripped)
        if parsed is not None:
            return parsed

        # The basic parser uses a non-greedy object regex, which can stop at
        # the first nested closing brace. Prefer the widest balanced-looking
        # object as a fallback for Region payloads containing point/bounds.
        start = stripped.find("{")
        end = stripped.rfind("}")
        ...
        # Repair only two ubiquitous formatting slips. Schema validation still
        # runs afterwards, so tolerance here cannot bypass Region safety checks.
    ...
    def query_region_preference(
        ...
        call_diagnostics = list(
            getattr(self._caller, "last_call_diagnostics", []) or []
        )
        for idx, text in enumerate(responses):
            parsed = self._extract_json_flexible(text)
            if parsed is None:
                logger.warning(
                    "Region response %d/%d: JSON extraction failed; preview=%s",
                    idx + 1,
                    len(responses),
                    self._safe_text_preview(text, 200),
                )
                continue
            ...
            if pref.parser_status == "ok":
                ...
                return pref
            logger.warning(
                "Region response %d/%d failed schema parsing: %s",
                idx + 1,
                len(responses),
                pref.parser_status,
            )
